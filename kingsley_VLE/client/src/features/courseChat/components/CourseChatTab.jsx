...
export default function CourseChatTab({ courseId, sectionId }) {
  ...
  const loadOlderMessages = async () => {
    ...
    try {
      const res = await courseChatApi.listMessages(courseId, sectionId, oldestId)
  ...
  const handleReact = async (messageId, type) => {
    try {
      const res = await courseChatApi.toggleReaction(courseId, sectionId, messageId, type)
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 170px)', minHeight: '400px' }}>
      {/* Top controls */}
