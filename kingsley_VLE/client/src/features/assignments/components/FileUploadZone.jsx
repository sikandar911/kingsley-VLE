...
function FileIcon({ name = '' }) {
  ...
  return (
    <svg className={`w-5 h-5 flex-shrink-0 ${colors[ext] || 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function FileUploadZone({
  ...
  const handleFiles = async (rawFiles) => {
    ...
    if (uploadedFiles.length + rawFiles.length > maxFiles) {
      setUploadError(`You can attach at most ${maxFiles} files.`)
      return
    }
    ...
    for (const file of filesToUpload) {
      if (!ALLOWED_TYPES[file.type]) {
        setUploadError(`File type not allowed: ${file.name}. Only PDF, DOCX, and PPTX files are accepted.`)
        return
      }
      if (file.size > MAX_FILE_SIZE) {
        setUploadError(`File size must be under 2 MB: ${file.name} is ${(file.size / 1024 / 1024).toFixed(2)} MB.`)
        return
      }
    }
      ...
      >
        ...
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-xs text-gray-400">PDF, Word (.docx), PowerPoint (.pptx) - Max {maxFiles} files, 2 MB each</p>
          </div>
        )}
      </div>
