...
const reminderInclude = {
  event: { select: { id: true, title: true, type: true, startTime: true, endTime: true, color: true } },
  assignment: {
    select: {
      id: true,
      title: true,
...
const buildAccessWhere = async (user) => {
  ...
  if (user.role === 'teacher') {
    const teacher = await prisma.teacherProfile.findUnique({
      ...
      select: { assignedCourses: { select: { courseId: true } }, assignedSections: { select: { id: true } } },
    })
    if (!teacher) return { id: 'none' }

    const courseIds = teacher.assignedCourses.map((c) => c.courseId)
    const sectionIds = teacher.assignedSections.map((s) => s.id)
  ...
  const sectionIds = student.enrollments.map((e) => e.sectionId).filter(Boolean)
...
export const createReminder = async (req, res) => {
  const { date, name, type, eventId, assignmentId, courseId, sectionId, semesterId, targetRole } = req.body
  ...
  if (targetRole && !validRoles.includes(targetRole)) {
    return res.status(400).json({ error: `targetRole must be one of: ${validRoles.join(', ')}` })
  }
  ...
  try {
    ...
    if (eventId) {
      ...
      if (!event) return res.status(400).json({ error: 'Referenced event not found' })
    }

    if (assignmentId) {
      const assignment = await prisma.assignment.findUnique({ where: { id: assignmentId } })
      if (!assignment) return res.status(400).json({ error: 'Referenced assignment not found' })
    }
...
export const updateReminder = async (req, res) => {
  ...
  try {
    const existing = await prisma.calendarReminder.findUnique({ where: { id } })
    if (!existing) return res.status(404).json({ error: 'Reminder not found' })

    const data = {}
    if (date !== undefined) {
      const parsed = new Date(date)
      if (isNaN(parsed)) return res.status(400).json({ error: 'Invalid date format' })
